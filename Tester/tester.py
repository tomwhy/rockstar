...
def main(args):
    for t in get_tests(args.Interpeter, args.Tests):
        ...
        test_result: test.TestResult = t[1].test()
        print(u"\u001b[2K", end="\r")  # clear the current line and return to the start of the line
        if test_result.success:
            print("[{0}V{1}] Test {2}{3}{1} {0}succeeded{1}.".format(GREEN, RESET, YELLOW, t[0]))
        else:
            print("[{0}X{1}] Test {2}{3}{1} {0}falied{1} with the following error:\n{4}".format(RED, RESET, YELLOW, t[0], test_result.msg))
        


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("Interpeter", help="The path to the rockstar interpeter")
